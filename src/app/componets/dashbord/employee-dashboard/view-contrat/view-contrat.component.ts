...
@Component({
  ...
  styleUrl: './view-contrat.component.scss'
})
export class ViewContratComponent {
      ...
      constructor(private modalService: NgbModal,private contratService: ContratService,private userService: UserService,private toastr: ToastrService, private suiviService: SuiviContratService,private sanitizer: DomSanitizer,private messageService: MessageService) { 
        this.currentUser = JSON.parse(localStorage.getItem('currentUser')!);
        this.selectedManager = this.currentUser.manager;
      ...
      openSendMessage() {
          this.newMessage = {};
          this.errorMessage = '';
          this.modalService.open(this.sendMessageModal, { ariaLabelledBy: 'modal-basic-title' });
}

      // Méthode pour envoyer le message
      sendMessage(modal: any) {
          if (!this.selectedManager || !this.messageContent.trim()) {
              this.toastr.warning('Please enter a message');
              return;
          }
      
          // Ici vous appellerez votre service d'envoi de message
          // Exemple fictif :
      ...
      loadPartnerContrats() {
    this.contratService.getContratsByPartner(this.currentUser.id).subscribe(
      (data: any[]) => {
        this.contrats = data;
      },
      (error) => {
      ...
      loadContratCount(): void {
        if (this.currentUser.role === 'PARTNER') {
       // Charger le nombre de contrats pour le partenaire connecté
       this.contratService.getContratCountByPartner(this.currentUser.id).subscribe({
      next: (count) => this.ContratCount = count,
      error: (err) => console.error('Error loading partner contract count', err)
       });
       } 
      }
